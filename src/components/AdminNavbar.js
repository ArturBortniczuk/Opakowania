// src/components/AdminNavbar.js - Zaktualizowany o zwijanie, przewijanie oraz Czat (Tylko dla Admina i Magazynu)
...
const AdminNavbar = ({
  ...
  const getRoleBadge = (role) => {
    ...
    return (
      <div className={`inline-flex items-center space-x-2 px-3 py-1.5 rounded-lg bg-gradient-to-r ${config.gradient} text-white text-xs font-semibold shadow-lg`}>
        ...
        {!isCollapsed && <span className="whitespace-nowrap overflow-hidden">{config.label}</span>}
      </div>
    );
  };
  ...
  return (
    <>
      ...
      {/* Modal czatu dla pracowników (tylko uprawnione role) */}
      {canAccessChat && (
        <AdminChatModal
          isOpen={isChatOpen}
