...
-- Dzięki SECURITY DEFINER funkcja wykonuje się z uprawnieniami właściciela (postgres),
-- omijając RLS dla klientów i widząc wszystkie zgłoszenia z danego miesiąca ze wszystkich firm.
...
DECLARE
  ...
  formatted_num TEXT;
BEGIN
  -- Jeśli request_number jest już ręcznie przekazany i prawidłowy, zachowujemy go
  ...
  -- Blokada współbieżności dla danego miesiąca, aby uniknąć równoczesnego przydzielenia tego samego numeru
  PERFORM pg_advisory_xact_lock(hashtext('return_req_num_' || ym_key));

  -- Obliczenie kolejnego numeru sekwencyjnego dla danego miesiąca (omija RLS)
  SELECT COALESCE(MAX(
    CASE 
