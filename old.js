...
const AdminDrumsList = ({ user, initialFilter = {} }) => {
  ...
  // NOWE: Stany formularza przed┼éu┼╝ania terminu zwrotu
  ...
  useEffect(() => {
    setCurrentPage(1); // Resetuj do pierwszej strony przy zmianie filtr├│w
  ...
  const handleSaveExtension = async () => {
    if (!customReturnDate) {
      alert('Prosz─Ö wybra─ç dat─Ö zwrotu.');
      return;
    }
    ...
    try {
      ...
      alert('Zapisano pomy┼Ťlnie nowy termin zwrotu!');
      
      // Od┼Ťwie┼╝ dane na li┼Ťcie
      ...
      console.error('B┼é─ůd podczas przed┼éu┼╝ania terminu:', err);
      alert('Wyst─ůpi┼é b┼é─ůd podczas zapisywania.');
    } finally {
  ...
  const handleSaveNote = async () => {
    setSavingNote(true);
    try {
      const nipToSave = selectedDrum.nip || urlClientNip; // Fallback do NIP z URL je┼Ťli potrzebny
      ...
      console.error('B┼é─ůd zapisu notatki:', err);
      alert('Nie uda┼éo si─Ö zapisa─ç notatki.');
    } finally {
      setSavingNote(false);
    }
  };
  ...
  const handleClearExtension = async () => {
    if (!window.confirm('Czy na pewno chcesz usun─ů─ç to przed┼éu┼╝enie i przywr├│ci─ç domy┼Ťlny termin?')) {
      return;
    }
    ...
    try {
      ...
      alert('Usuni─Öto przed┼éu┼╝enie i przywr├│cono domy┼Ťlny termin.');
      ...
      console.error('B┼é─ůd usuwania przed┼éu┼╝enia:', err);
      alert('Nie uda┼éo si─Ö przywr├│ci─ç terminu domy┼Ťlnego: ' + err.message);
    } finally {
      setSavingExtension(false);
    }
  };
  ...
  const handleImportFile = async () => {
    if (!window.confirm('ÔÜá´ŞĆ UWAGA: To zast─ůpi WSZYSTKIE dane w tabeli drums. Kontynuowa─ç?')) {
      return;
    }
    ...
    input.onchange = async (e) => {
      ...
      if (!fileName.endsWith('.csv') && !fileName.endsWith('.xlsx') && !fileName.endsWith('.xls')) {
        alert('ÔŁî Prosz─Ö wybra─ç plik CSV lub XLSX');
        return;
      }
      ...
      if (file.size > maxFileSize) {
        alert(`ÔŁî Plik jest za du┼╝y: ${Math.round(file.size / 1024 / 1024)}MB. Maksymalny rozmiar: 50MB`);
        return;
      }

      console.log(`­čôü Plik: ${file.name}, rozmiar: ${Math.round(file.size / 1024)}KB`);
      ...
      try {
        ...
        if (fileName.endsWith('.csv')) {
          ...
          const csvContent = await new Promise((resolve, reject) => {
            ...
            reader.onload = (e) => {
              try {
                ...
                let decodedText = '';
                // 1. Spr├│buj zdekodowa─ç jako UTF-8 z fatal: true
                try {
                  const decoder = new TextDecoder('utf-8', { fatal: true });
                  decodedText = decoder.decode(uint8);
                  console.log('Ôťů CSV odczytany pomy┼Ťlnie jako UTF-8');
                } catch (utf8Error) {
                  console.warn('ÔÜá´ŞĆ Niepoprawny format UTF-8, pr├│ba dekodowania jako Windows-1250...', utf8Error);
                  // 2. Je┼Ťli UTF-8 rzuci b┼é─ůd, spr├│buj Windows-1250 (CP1250)
                  const decoder = new TextDecoder('windows-1250');
                  decodedText = decoder.decode(uint8);
                  console.log('Ôťů CSV odczytany pomy┼Ťlnie jako Windows-1250');
                }

                // Wykryj obecno┼Ť─ç polskich znak├│w diakrytycznych
                hasPolish = /[─ů─ç─Ö┼é┼ä├│┼Ť┼║┼╝─ä─ć─ś┼ü┼â├ô┼Ü┼╣┼╗]/.test(decodedText);
                console.log(`­čçÁ­čç▒ Czy wykryto polskie znaki w pliku CSV: ${hasPolish ? 'Tak' : 'Nie'}`);
            ...
            reader.onerror = () => reject(new Error('B┼é─ůd odczytu CSV'));
            reader.readAsArrayBuffer(file);
          });

          // SPRAWD┼╣ LICZB─ś LINII
          const lines = csvContent.split('\n').filter(line => line.trim());
          console.log(`­čôŐ CSV ma ${lines.length} linii`);

          if (lines.length > 10000) {
            const proceed = window.confirm(
              `ÔÜá´ŞĆ UWAGA: Plik ma ${lines.length} linii.\n\n` +
              'Du┼╝e pliki mog─ů powodowa─ç problemy z pami─Öci─ů.\n' +
              'Zalecamy podzielenie na mniejsze cz─Ö┼Ťci.\n\n' +
              'Czy kontynuowa─ç?'
            );
            if (!proceed) {
          ...
          console.log('­čôŐ Tryb XLSX...');

          if (file.size > 50 * 1024 * 1024) {
            alert('ÔŁî Plik XLSX jest za du┼╝y. Maksymalny rozmiar dla Excel: 50MB');
            setImportLoading(false);
            return;
          }
          ...
          const base64 = await new Promise((resolve, reject) => {
            ...
            reader.onerror = () => reject(new Error('B┼é─ůd odczytu XLSX'));
        ...
        if (!response.ok) {
          ...
          console.error('ÔŁî B┼é─ůd HTTP:', response.status, errorText);
          throw new Error(`B┼é─ůd serwera ${response.status}: ${errorText}`);
        }

        const result = await response.json();
        console.log('­čôŁ Odpowied┼║ serwera:', result);
        ...
        if (result.success) {
          const message = `Ôťů SUKCES!\n\n${result.message}\n\n` +
            `­čôŐ Format: ${fileName.endsWith('.csv') ? 'CSV' : 'XLSX'}\n` +
            `­čçÁ­čç▒ Polskie znaki: ${hasPolish ? 'OK' : 'Brak'}\n` +
            `­čôŽ Importowano: ${result.imported} rekord├│w\n` +
            `­čĆó Dodano firm: ${result.companiesAdded || 0}\n` +
            `ÔÜá´ŞĆ Pomini─Öto: ${result.skipped || 0} b┼é─Ödnych`;

          alert(message);
          await handleRefresh();
        } else {
          throw new Error(result.message || 'Nieznany b┼é─ůd importu');
        }

      } catch (error) {
        console.error('ÔŁî B┼é─ůd importu:', error);

        let userMessage = `ÔŁî B┼é─ůd importu: ${error.message}`;

        if (error.message.includes('Maximum call stack')) {
          userMessage = `ÔŁî Plik jest za du┼╝y lub zawiera b┼é─Ödy formatowania.\n\n` +
            `­čĺí Rozwi─ůzania:\n` +
            `ÔÇó Podziel plik na mniejsze cz─Ö┼Ťci (max 2000 wierszy)\n` +
            `ÔÇó Sprawd┼║ czy nie ma uszkodzonych znak├│w w danych\n` +
            `ÔÇó U┼╝yj XLSX zamiast CSV dla lepszej kompatybilno┼Ťci`;
        } else if (error.message.includes('Failed to fetch')) {
          userMessage += '\n\n­čöž Sprawd┼║ po┼é─ůczenie internetowe lub spr├│buj ponownie za chwil─Ö.';
        }
  ...
  useEffect(() => {
    ...
    // Je┼╝eli zmienili┼Ťmy stron─Ö bo by┼éa poza zakresem, zaaktualizuj stan ┼╝eby UI si─Ö zgadza┼é
  ...
  const DrumCard = ({ drum, index }) => {
    ...
    const isReturned = reportedCechas.has(drum.cecha) || reportedCechas.has(drum.kod_bebna) || drum.status === 'reported' || drum.status === 'Zg┼éoszone do zwrotu';
          ...
          <div className="flex items-center space-x-3 min-w-0 flex-1">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-gray-600 text-sm truncate">
                {drum.cecha ? `${drum.kod_bebna} ÔÇó ${drum.rozmiar_bebna ? `FI ${drum.rozmiar_bebna}` : 'Brak rozmiaru'}` : (drum.rozmiar_bebna ? `FI ${drum.rozmiar_bebna}` : 'Brak rozmiaru')}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3 flex-1">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className={`text-sm font-medium ${isSupplierOverdue ? 'text-red-600 font-bold' : 'text-gray-900'}`}>
                {drum.DATA_ZWROTU_DO_DOSTAWCY ?
                  ...
                  <span className="text-indigo-600 font-semibold">W┼éasny</span>
                }
              </span>
            </div>
          )}
          ...
          {!isAdmin && (
            <div className="pt-3 mt-3 border-t border-gray-100 space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-500">Op┼éacony?</span>
              ...
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-500">Termin p┼éatno┼Ťci</span>
                <span className="text-sm font-medium text-gray-900">
              ...
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-500">Kabel na b─Öbnie</span>
              ...
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-500">Ilo┼Ť─ç kabla</span>
                <span className="text-sm font-medium text-gray-900">
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">B┼é─ůd ┼éadowania</h2>
  ...
  const handleExportExcel = async () => {
    try {
      // Pobierz wszystko (do 10 000 rekord├│w) z obecnymi filtrami
      ...
      const exportData = result.data.map(drum => ({
        ...
        'Data przyj─Öcia na stan': drum.data_przyjecia_na_stan,
        'Termin zwrotu (klient)': drum.clientReturnDeadline,
        'Termin zwrotu (dostawca)': drum.data_zwrotu_do_dostawcy,
        'Przed┼éu┼╝ony': drum.isExtended ? 'Tak' : 'Nie',
        'Dni w posiadaniu': drum.daysInPossession,
        'Op┼éacony': drum.czy_zaplacona,
        'Termin p┼éatno┼Ťci': drum.termin_platnosci,
        'Cena netto': drum.cena_netto_bebna || drum.CENA_NETTO_BEBNA,
        'Kabel na b─Öbnie': drum.nawiniety_kabel,
        'Ilo┼Ť─ç kabla [m]': drum.ilosc_kabla,
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, "B─Öbny");
      XLSX.writeFile(workbook, `Bebny_Eksport_${new Date().toISOString().split('T')[0]}.xlsx`);
    } catch (err) {
      console.error('B┼é─ůd eksportu:', err);
      alert('Nie uda┼éo si─Ö wyeksportowa─ç pliku.');
    }
  };
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">Wszystkie b─Öbny</h1>
              <p className="text-gray-600">
                Zarz─ůdzaj baz─ů {drumsData.pagination.total} b─Öbn├│w
              </p>
            </div>
          ...
          {/* Stats Cards - POPRAWIONE: u┼╝ywa pagination.total zamiast filteredAndSortedDrums.length */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
            <div className="bg-white/80 backdrop-blur-lg rounded-2xl p-6 shadow-lg border border-blue-100">
              <div className="flex items-center">
                <Package className="w-8 h-8 text-blue-600" />
                <div className="ml-4">
                  <p className="text-sm font-medium text-gray-600">Wszystkie b─Öbny</p>
            ...
            <div className="bg-white/80 backdrop-blur-lg rounded-2xl p-6 shadow-lg border border-yellow-100">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <p className="text-sm font-medium text-gray-600">Zbli┼╝a si─Ö termin</p>
            ...
            <div className="bg-white/80 backdrop-blur-lg rounded-2xl p-6 shadow-lg border border-indigo-100">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <p className="text-sm font-medium text-gray-600">Przed┼éu┼╝one</p>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="relative">
                  ...
                  <input
                    type="text"
                    placeholder="Dane b─Öbna (cecha, faktura)..."
                  ...
                  {showSizesMenu && (
                    <div className="absolute z-50 mt-2 w-full max-h-60 overflow-auto bg-white border border-gray-200 rounded-xl shadow-xl p-2">
                      {availableSizes.length > 0 ? (
                        ...
                        <div className="p-2 text-sm text-gray-500 text-center">Brak rozmiar├│w do wyboru</div>
                      )}
              ...
              <div className={`grid grid-cols-1 ${isSalesperson ? 'md:grid-cols-4' : 'md:grid-cols-5'} gap-4`}>
                <div className="flex items-center p-3 border border-gray-300 rounded-xl bg-white hover:bg-gray-50 transition-colors text-sm">
                  <label className="flex items-center w-full cursor-pointer select-none">
                    <input
                      ...
                      className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                    />
                    <span className="ml-2 text-xs font-semibold text-gray-700 leading-tight">Tylko zg┼éoszone zwroty</span>
                  </label>
                </div>
                {!isSalesperson && (
                  <select
                  ...
                  >
                    ...
                    <option value="wydane">Wydane u klient├│w</option>
                ...
                >
                  ...
                  <option value="active">Bez przekrocze┼ä</option>
                  <option value="due-soon">Zbli┼╝a si─Ö termin</option>
                ...
                >
                  ...
                  <option value="due-soon">Zbli┼╝a si─Ö termin</option>
                  <option value="overdue">Przeterminowane</option>
                  <option value="extended">Przed┼éu┼╝one terminy</option>
                </select>
                ...
                <select
                  ...
                  className="p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                >
                  <option value="all">P┼éatno┼Ť─ç: Wszystkie</option>
                  <option value="paid">Op┼éacone</option>
                  <option value="unpaid">Nieop┼éacone</option>
                  <option value="no_invoice">Brak faktury</option>
                  <option value="overdue_payment">Zaleg┼ée po terminie</option>
                </select>
              </div>
            </div>
          </div>
        ...
        {/* Results - ZMIENIONE: u┼╝ywa drumsData.data zamiast filteredAndSortedDrums */}
        {drumsData.data.length > 0 ? (
                ...
                <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                  <div>
                    <p className="text-sm text-gray-700">
                      Wy┼Ťwietlane <span className="font-medium">{((drumsData.pagination.page - 1) * drumsData.pagination.limit) + 1}</span> do{' '}
                      <span className="font-medium">
                        {Math.min(drumsData.pagination.page * drumsData.pagination.limit, drumsData.pagination.total)}
                      </span>{' '}
                      z <span className="font-medium">{drumsData.pagination.total}</span> b─Öbn├│w
                    </p>
                  </div>
                  <div>
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-2">
              {searchTerm || filterStatus !== 'all' ? 'Nie znaleziono b─Öbn├│w' : 'Brak b─Öbn├│w'}
            </h3>
            <p className="text-gray-600 mb-6">
              {searchTerm || filterStatus !== 'all'
                ? 'Spr├│buj zmieni─ç kryteria wyszukiwania lub filtry'
                : 'Nie znaleziono b─Öbn├│w przypisanych do Twojego konta'
              }
            </p>
            {(searchTerm || filterStatus !== 'all') && (
              <button
            ...
            >
              <div className="p-6 border-b border-gray-200">
                <div className="flex items-center justify-between">
                  <h2 className="text-2xl font-bold text-gray-900">Szczeg├│┼éy b─Öbna {selectedDrum.cecha || selectedDrum.kod_bebna}</h2>
                  <button
                    onClick={handleCloseModal}
              ...
              <div className="p-6 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900 mb-4 border-b pb-2 border-gray-100">Informacje o b─Öbnie</h3>
                    <div className="space-y-3 bg-gray-50/50 p-4 rounded-xl border border-gray-100">
                      <div className="flex justify-between">
                        <span className="text-sm font-medium text-gray-500">Kod b─Öbna</span>
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold text-gray-900 mb-4 border-b pb-2 border-gray-100">P┼éatno┼Ťci i kable</h3>
                      <div className="space-y-3 bg-emerald-50/30 p-4 rounded-xl border border-emerald-100/50">
                        <div className="flex justify-between items-center">
                          <span className="text-sm font-medium text-gray-500">Op┼éacony?</span>
                        ...
                        <div className="flex justify-between items-center">
                          <span className="text-sm font-medium text-gray-500">Termin p┼éatno┼Ťci</span>
                          <span className="text-sm font-medium text-gray-900">{selectedDrum.termin_platnosci || 'Brak terminu'}</span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-sm font-medium text-gray-500">Kabel na b─Öbnie</span>
                        ...
                        <div className="flex justify-between items-center">
                          <span className="text-sm font-medium text-gray-500">Ilo┼Ť─ç kabla</span>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                    ...
                    <div className="bg-gray-50 p-3 rounded-lg border border-gray-100 text-center">
                      <p className="text-xs text-gray-500 mb-1">Przyj─Öcie</p>
                      <p className="font-semibold text-sm text-gray-900">
                    ...
                    <div className="bg-blue-50/50 p-3 rounded-lg border border-blue-100 text-center">
                      ...
                      <p className="font-semibold text-sm text-blue-900">
                        {selectedDrum.data_zwrotu_do_dostawcy ? new Date(selectedDrum.data_zwrotu_do_dostawcy).toLocaleDateString('pl-PL') : <span className="text-indigo-600">W┼éasny</span>}
                      </p>
                    </div>
                ...
                <div className="mt-6">
                  ...
                  <div className="bg-gray-50/50 p-4 rounded-xl border border-gray-200">
                    <textarea
                      ...
                      placeholder="Wpisz notatk─Ö do tego b─Öbna..."
                ...
                {/* Indywidualne przed┼éu┼╝enie terminu zwrotu (Tylko dla Admina) */}
                {currentUser?.role?.toLowerCase() === 'admin' && (
                  <div className="mt-6 pt-6 border-t border-indigo-100 bg-indigo-50/30 -mx-6 -mb-6 p-6 rounded-b-2xl">
                    <h3 className="text-lg font-bold text-indigo-900 mb-3 flex items-center space-x-2">
                      ...
                      <span>Indywidualne przed┼éu┼╝enie terminu zwrotu</span>
                    </h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
                      <div>
                        ...
                        <DatePicker
                          ...
                          placeholderText="Wybierz now─ů dat─Ö"
                      ...
                      <div className="md:col-span-2">
                        ...
                        <input
                          ...
                          placeholder="np. Ustalone z klientem, op├│┼║nienie inwestycji do ko┼äca roku."
                        />
                      </div>
                    </div>
                      ...
                      <div>
                        {selectedDrum.isExtended && (
                          <p className="text-xs text-gray-500">
                            Wprowadzi┼é: <span className="font-semibold">{selectedDrum.extensionCreatedBy || "Brak"}</span>
